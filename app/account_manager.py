...
class AccountManager:
    ...
    def load_all_accounts(self) -> Dict[str, Dict]:
        ...
        logger.info("Loading account configurations", accounts_dir=str(self.accounts_dir))
        
        if not self.accounts_dir.exists():
            logger.warning("Accounts directory does not exist", path=str(self.accounts_dir))
            return {}
        ...
        for account_file in self.accounts_dir.glob("*.json"):
            try:
                ...
                logger.debug("Loaded account configuration", 
        ...
        if accounts:
            ...
            logger.info("Successfully loaded accounts", count=len(accounts), accounts=list(accounts.keys()))
        else:
    ...
    def get_account(self, account_id: str) -> Optional[Dict]:
        ...
        if not account:
            logger.warning("Account not found", account_id=account_id, available_accounts=list(accounts.keys()))
    ...
    def validate_account_config(self, config: Dict) -> bool:
        ...
        for field in required_fields:
            if field not in config:
                raise ConfigurationError(f"Missing required field in account config: {field}")
        ...
        if not account_id.replace("_", "").replace("-", "").isalnum():
            raise ConfigurationError("account_id must contain only alphanumeric characters, hyphens, and underscores")
        
        # Validate twitter_credentials
        twitter_creds = config["twitter_credentials"]
        required_creds = ["api_key", "api_secret", "access_token", "access_token_secret", "bearer_token"]
        ...
        for cred in required_creds:
            ...
            if twitter_creds[cred].startswith("REPLACE_WITH_ACTUAL_"):
                logger.warning("Twitter credentials contain placeholder values", 
                             account_id=account_id, 
                             credential=cred)
        ...
        if not isinstance(config["vector_collection"], str) or not config["vector_collection"].strip():
            raise ConfigurationError("vector_collection must be a non-empty string")
        
        logger.debug("Account configuration validated successfully", account_id=account_id)
        return True
    ...
    def save_account(self, account_config: Dict) -> bool:
        ...
        try:
            ...
            logger.info("Saved account configuration", account_id=account_id, file=str(account_file))
            ...
            logger.error("Failed to save account configuration", 
    ...
    def delete_account(self, account_id: str) -> bool:
        ...
        try:
            ...
            if not account_file.exists():
                logger.warning("Account file does not exist", account_id=account_id, file=str(account_file))
                return False
            ...
            logger.error("Failed to delete account configuration", account_id=account_id, error=str(e))
            return False
