...
class ContentFilter:
    ...
    def __init__(self):
        ...
        self.use_openai_moderation = content_filter_config.get("use_openai_moderation", True)
        ...
        self.profanity_words = {
            ...
            "bastard", "slut", "whore", "dick", "cock", "pussy", "cunt"
        }
    ...
    def _basic_safety_check(self, text: str) -> bool:
        ...
        for pattern in self.inappropriate_patterns:
            if re.search(pattern, text_lower):
                logger.warning("Content rejected for inappropriate pattern", pattern=pattern)
                self._log_filter_event("inappropriate_pattern", text, f"Matches pattern: {pattern}")
                return False
        
        # Check for political content
        political_words_found = [word for word in self.political_keywords if word in text_lower]
        if political_words_found:
            logger.warning("Content rejected for political content", words=political_words_found)
            self._log_filter_event("political_content", text, f"Contains: {', '.join(political_words_found)}")
            return False
        ...
        if caps_ratio > 0.5 and len(text) > 20:
            ...
            self._log_filter_event("excessive_caps", text, f"Caps ratio: {caps_ratio:.2f}")
            return False
        ...
        if len(words) > 5:
            ...
            if len(unique_words) / len(words) < 0.5:  # Less than 50% unique words
                ...
                self._log_filter_event("repetitive_text", text, "Too much word repetition")
                return False
    ...
    def _openai_moderation_check(self, text: str) -> bool:
        ...
        try:
            ...
            if result.flagged:
                ...
                logger.warning("Content flagged by OpenAI moderation", 
